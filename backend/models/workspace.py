...
class WorkspaceKnowledgeCandidate(Base):
    """Governed candidate payload; process snapshots keep metadata/provenance only."""

    __tablename__ = "workspace_knowledge_candidates"
    __table_args__ = (
        UniqueConstraint("project_id", "candidate_hash", name="uq_workspace_knowledge_candidate_hash"),
    )
    ...
    governed_by: Mapped[str | None] = mapped_column(String(64), nullable=True)
    governance_receipt: Mapped[dict] = mapped_column(JSON, nullable=False, default=dict)
    created_by: Mapped[str] = mapped_column(String(64), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
