...
【核心任务】你的第一职责不是"怎么做"，而是"**值不值得做**"——用市场洞察 + deep research 的方法论，判断这个产品需求是否合理、是否值得投入。按以下逻辑执行后再产出数据：

一、需求合理性初判：这个需求解决什么问题？目标用户是谁？市场上是否已有同类？
二、外部情报（deep research，用搜索工具，每条标注来源）：
   - 竞争对手有没有做？做得怎么样？（找 2-5 个代表）
   - 市场空间有多大？（行业数据/趋势）
   - 对手优势在哪？（产品/渠道/生态/价格）
   - 对手劣势在哪？（短板/未满足需求）
   - 我们入局是否合适？（时机/切入点/差异化）
三、内部盘点（检索知识库；【搜不到必须写"知识库暂无相关信息"，绝不编造】）：
   - 商业重心？基本盘？优势能力？
   - 战略上是否支持？流程上是否支持？有没有对应的人？
四、结论：值得做 / 不值得做 / 有条件地做（说明条件）+ 依据
...
二、外部情报搜集（deep research，用搜索工具获取真实资料，每条标注来源）
- 竞争对手有没有做？做得怎么样？（找 2-5 个代表性对手/产品）
- 市场空间有多大？（行业报告、市场规模数据、增长趋势）
- 对手的优势在哪？（产品、渠道、生态、价格）
...
{{
  ...
  "external_tasks": [
    "外部调研任务1（如：竞品A产品与市场份额分析）",
    "外部调研任务2（如：市场规模与增长趋势研究）",
    "外部调研任务3（如：对手优劣势对比）"
  ],
  "external_details": [
    "任务1的具体调研发现（含来源/数据）",
...
注意：各个 tasks 数组和其对应的 details 数组的长度必须相同。内部资料搜不到时，internal_details 必须如实写"知识库中暂无相关信息"，不要编造。
"""
    else:
        prompt = f"""
