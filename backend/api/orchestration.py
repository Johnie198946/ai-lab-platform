...
- 用户说"想做外卖平台"，就只分析外卖平台这个具体业务（外卖行业竞品、外卖市场数据、做外卖平台的关键能力），禁止输出与外卖无关的内容（如 AI 平台、智能体、TokenOps、超聚变业务、行业通用分析等）。
...
一、需求合理性初判（围绕「{body.goal}」）：这个需求解决什么问题？目标用户是谁？市场上是否已有同类？红海还是蓝海？

二、外部情报（deep research 围绕「{body.goal}」所属行业，每条标注来源）：
