...
def _research_stage_context(stage: str) -> str:
    ...
    return common + (
        ...
        "Do not spend phase one verifying tangential concept-origin dates or every promotional "
        "number. Link the original and every external source actually used with clickable URLs "
        "in the answer. Search snippets are discovery evidence, not proof that you read the "
        "primary source: fetch the relevant primary passage before saying it confirms a claim, "
        "or label the evidence snippet-only/unverified. Use execute_code for any recalculation; "
        "never claim arithmetic was checked without actual tool output. "
