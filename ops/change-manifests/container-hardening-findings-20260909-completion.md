...
- The final write probe found a restored root-owned `.incremental-compile.lock`. The updater now repairs that known Vault runtime lock to `quantumn-hermes:quantumn-hermes 0600` along with the Vault root before and after the Hermes restart.
