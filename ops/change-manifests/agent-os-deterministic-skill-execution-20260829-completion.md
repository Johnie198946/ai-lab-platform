...
> [!summary]
> 将本地 Agent OS 的 Skill-first 从“提示模型自行调用”改为 Runtime 在 `pre_llm_call` 阶段通过 Hermes 原生 `ctx.dispatch_tool("skill_view", ...)` 确定性执行，并把真实成功结果注入本轮上下文；Agency 仍使用 Hermes 原生 `delegate_task`，参数合同修正为当前 `tasks[]` schema。
