...
- TestFlight: not uploaded. Build 26 is required for the durable replay/card fix and the iOS prewarm trigger; build 25 cannot receive those client changes from a server deployment.
- remaining_risks: real authenticated model TTFT must be measured manually because the smoke check intentionally does not invoke a paid model or write user data. Production logs still report pre-existing Hermes plugin-layout and old SQLite WAL warnings; neither prevented worker startup or health checks.
